...
void Font::Draw(float x, float y, const wchar_t* string, float size_x, float size_y,float r,float g,float b,float a)
{
	...
	for (size_t i = 0; i < length; ++i)
	{
		...
		float positionX = x * size_x + info.xoffset ;// + 0.5f;
		float positionY = y * size_y + info.yoffset;// + 0.5f;
		...
		currentVertex[1].position.x = positionX + (info.width * size_x);
		...
		currentVertex[2].position.y = positionY + (info.height * size_y);
		...
		currentVertex[3].position.x = positionX + (info.width * size_x);
		currentVertex[3].position.y = positionY + (info.height * size_y);
